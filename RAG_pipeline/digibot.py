...
# def validate_text_input(user_input):
#     """Validate that the input is text and not empty"""
#     if not isinstance(user_input, str):
#         return False, "Input must be text (string format)."
    
#     if not user_input.strip():
#         return False, "Input cannot be empty. Please provide some text."
    
#     # Check for non-printable characters or binary data
#     try:
#         user_input.encode('utf-8')
#     except UnicodeEncodeError:
#         return False, "Input contains invalid characters. Please use plain text."
...
#     # Validate user input
#     is_valid, validation_message = validate_text_input(user_input)
#     if not is_valid:
#         print(f"Validation failed: {validation_message}")
#         return validation_message, "Validation Error"
